...
class InquiryPawningExport implements FromCollection, WithHeadings
{
    /**
        ...
        if($this->isAdmin){
            if($this->inquiryState == 'ANY_STATE'){
                ...
                return Inquiry::select('inquiries.id', 'inquiries.inquiry_status', 'customer_profiles.first_name', 'customer_profiles.last_name', 'customer_profiles.email', 'customer_profiles.mobile_number', 'customer_profiles.dob', 'customer_profiles.address', 'districts.district_name', 'customer_profiles.nic', 'inquiry_pawnings.inquiry_step', 'inquiry_pawnings.pawned_elsewhere', 'inquiry_pawnings.amount')
                            ...
                            ->join('districts', 'districts.id', '=', 'customer_profiles.district')
                            ->where('inquiries.inquiry_type', 'PAWNING')
                            ->where('inquiry_pawnings.inquiry_step', $this->inquiryState)
                            ->get();
            }
        }else{
            if($this->inquiryState == 'ANY_STATE'){
