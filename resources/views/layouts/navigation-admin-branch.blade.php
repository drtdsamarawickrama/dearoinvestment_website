...
<header class="w-full flex flex-col z-50">
        ...
        <nav class="md:h-full flex items-center justify-start sm:max-w-screen-2xl mx-auto">
            <div class="nav-links absolute md:static mt-0 text-theme-blue bg-theme-orange-ribbon md:bg-transparent min-h-[50vh] md:min-h-fit w-full md:w-auto left-0 top-[-100%] flex items-center sm:px-20">
                <ul class="flex flex-col md:flex-row md:items-center gap-8 md:gap-[2vw] text-md font-bold">
                    <li>
                        @if(isset($manager))
                            <a href="{{ route('admin.dashboard.load') }}" class="text-theme-gray mr-8">Dearo Investment | Branch Manager - {{ $manager->manager_name }}</a>
                        @else
    ...
    <div  id="desktop-header-2" class="h-12 w-full bg-theme-orange menu-holder-2 hidden lg:flex justify-start">
        <nav class="w-full flex items-center justify-start sm:max-w-screen-2xl mx-auto">
            <div class="nav-links-2 absolute md:static mt-0 text-theme-blue bg-theme-orange md:bg-transparent min-h-fit w-full md:w-auto left-0 top-[-100%] flex items-center sm:px-20">
                <ul class="flex flex-col md:flex-row md:items-center gap-8 md:gap-[2vw] text-md font-bold">
                    <li class="w-auto h-full border-white hover:border-b-[1px]">
                        <a class="pr-4 py-2" href="{{ route('branch.dashboard.load') }}" >Dashboard</a>
                    </li>
                    <li class="w-auto h-full border-white hover:border-b-[1px]">
                        <a class="pr-4 py-2" href="{{ route('branch.inquiries.lease.load') }}" >Leasing Inquiries</a>
                    </li>
                    <li class="w-auto h-full border-white hover:border-b-[1px]">
                        <a class="pr-4 py-2" href="{{ route('branch.inquiries.fd.load') }}" >FD Inquiries</a>
                    </li>
                    <li class="w-auto h-full border-white hover:border-b-[1px]">
                        <a class="pr-4 py-2" href="{{ route('branch.inquiries.insurance.load') }}" >Insurance Inquiries</a>
                    </li>
                    <li class="w-auto h-full border-white hover:border-b-[1px]">
                        <a class="pr-4 py-2" href="{{ route('branch.inquiries.pawning.load') }}" >Pawning Inquiries</a>
                    </li>
                    <li class="w-auto h-full border-white hover:border-b-[1px]">
                        <a class="pr-4 py-2" href="#" >Profile</a>
                    </li>
                    <li class="w-auto h-full border-white hover:border-b-[1px]">
    ...
    <div  id="mobile-header-2" class="h-auto bg-theme-orange menu-holder-1 lg:hidden absolute top-[55vh] left-0 w-10/12 md:w-1/2 max-w-[500px] z-10 hidden rounded-br-lg">
        <nav class="flex items-center justify-start mx-auto">
            <div class="nav-links mt-0 text-theme-blue bg-theme-orange min-h-[35vh] pb-8 w-full left-0 flex flex-col items-start rounded-br-lg">
