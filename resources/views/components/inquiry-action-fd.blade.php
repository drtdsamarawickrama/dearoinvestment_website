    ...
    <div class="w-full h-full sm:max-w-screen-2xl mx-auto px-4 sm:px-10 md:px-20 pt-6 sm:pt-0">
        ...
        <div class="w-full h-auto sm:w-2/3 md:w-1/2 mt-5">
            <div class="w-full lg:w-2/3">
                ...
                <form action="{{ route('dearo.inquiry.fd.application.state.update') }}" method="post" class="p-0 m-0">
                    @csrf
                    ...
                    <div class="w-full flex items-center justify-start">
                        <select name="fd_inquiry_status" class="h-10 border-theme-gray text-md">
                            @foreach($fd_details->getAdminInquirySteps() as $stepIdentifier)
                                @if($fd_details->inquiry_step == $stepIdentifier)
                                    <option value="{{ $stepIdentifier }}" selected>{{ $fd_details->getAdminInquiryStepLabel($stepIdentifier) }}</option>
                                @else
                                    <option value="{{ $stepIdentifier }}">{{ $fd_details->getAdminInquiryStepLabel($stepIdentifier) }}</option>
                                @endif
                            @endforeach
                        </select>
