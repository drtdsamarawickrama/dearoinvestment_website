<div class="h-auto max-h-[400px] sm:h-[280px] relative top-[-400px] sm:top-[-280px] left-0 bg-gradient-to-r from-white sm:bg-none">
    <div class="w-full h-full sm:max-w-screen-2xl mx-auto px-4 sm:px-10 md:px-20 pt-6 sm:pt-0">
        <p class="text-3xl lg:text-4xl text-theme-blue font-light mt-10">Leasing for <span class="font-bold">{{ $customer_profile->first_name }}</span></p>
        @php 
        ...
        <div class="w-full h-auto sm:w-2/3 md:w-1/2 mt-5">
            <div class="w-full lg:w-2/3">
                ...
                <form action="{{ route('dearo.inquiry.lease.application.state.update') }}" method="post" class="p-0 m-0">
                    @csrf
                    <input type="hidden" name="inquiry_id" value="{{ $inquiry_general->id }}" />
                    <div class="w-full flex items-center justify-start">
                        <select name="lease_inquiry_status" class="h-10 border-theme-gray text-md">
                            @foreach($lease_details->getAdminInquirySteps() as $stepIdentifier)
                                @if($lease_details->inquiry_step == $stepIdentifier)
                                    <option value="{{ $stepIdentifier }}" selected>{{ $lease_details->getAdminInquiryStepLabel($stepIdentifier) }}</option>
                                @else
                                    <option value="{{ $stepIdentifier }}">{{ $lease_details->getAdminInquiryStepLabel($stepIdentifier) }}</option>
                                @endif
                            @endforeach
                        </select>
