...
<head>
    ...
    <title>Leasing Inquiries | Dearo Branch Admin</title>
</head>
<body class="font-lato h-screen bg-white flex flex-col">
    ...
    <div class="w-full py-6">
        <div class="w-full sm:max-w-screen-2xl mx-auto px-4 sm:px-10 md:px-20">
            <p class="text-3xl lg:text-4xl text-theme-blue font-bold mt-10">Leasing Inquiries</p>
            <p class="text-2xl text-theme-orange font-bold mt-2">Dearo Branch - <span class="font-light text-3xl">{{ $branch->branch_name }}</span></p>
        </div>
    </div>
    ...
    <div class="w-full py-5 mb-auto">
        <div class="w-full sm:max-w-screen-2xl mx-auto px-4 sm:px-10 md:px-20">
            ...
            <div class="w-full flex justify-between">
                <div class="w-auto">
                    <form action="{{ route('branch.inquiries.lease.search') }}" method="GET" class="p-0 m-0">
                ...
                <div class="w-auto flex items-center justify-end gap-4">
                    <a href="{{ route('branch.inquiries.lease.filter',['criteria' => 'payment-arrears']) }}">
                        <button type="button" class="h-min {{ (Request::is('*payment-arrears'))?'bg-theme-gray text-white':'bg-theme-blue-lite text-theme-gray' }} font-bold px-4 py-2">Payment Arrears</button>
                    </a>
                    <a href="{{ route('branch.inquiries.lease.filter',['criteria' => 'vehicle-missing']) }}">
                        <button type="button" class="h-min {{ (Request::is('*vehicle-missing'))?'bg-theme-gray text-white':'bg-theme-blue-lite text-theme-gray' }} font-bold px-4 py-2">Vehicle Missing</button>
                    </a>
                    <div class="w-auto">
                        <form action="{{ route('branch.inquiries.lease.filter.by.state') }}" method="GET" class="p-0 m-0">
                            <div class="w-auto flex items-center justify-start gap-4">
                                <select id="search_inquiry_status" name="inquiry_status" class="h-10 mt-1 border-b-theme-blue border-0 border-b-[1px] text-sm w-full">
                                    ...
                                    <option value="DEARO1_INITIAL_APPROVAL">Initial Approval Done</option>
                                    <option value="DEARO2_CUSTOMER_VISIT">Visited Customer</option>
                                    <option value="DEARO3_SUBMIT_FOR_APPROVAL">Submitted for Manager Approval</option>
                                    <option value="DEARO4_APPLIED_FOR_MANAGER_APPROVAL">Approved by Manager</option>
                                    <option value="DERO5_PAYMENT_RELEASED">Payment Released</option>
                                </select>
            ...
            <div class="w-full flex mt-5">
                <div class="w-auto flex items-center justify-end gap-4">
                    <div class="w-auto mt-8">
                        <form action="{{ route('branch.inquiry.lease.export') }}" method="GET" class="p-0 m-0">
